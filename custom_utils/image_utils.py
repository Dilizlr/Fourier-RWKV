...
def torchPSNR(prd_img, tar_img):
    # prd_img = torch.clamp(prd_img, 0, 1)
...
def torchSSIM(prd_img, tar_img):
    return ssim(prd_img, tar_img, data_range=1.0, size_average=True)
...
def torchPSNR_R(prd_img, tar_img):
    psnr_val = 10 * torch.log10(1 / f.mse_loss(prd_img, tar_img))
    return psnr_val

def torchSSIM_R(prd_img, tar_img, H, W):
    down_ratio = max(1, round(min(H, W) / 256))	
    ssim_val = ssim(f.adaptive_avg_pool2d(prd_img, (int(H / down_ratio), int(W / down_ratio))), 
                    f.adaptive_avg_pool2d(tar_img, (int(H / down_ratio), int(W / down_ratio))), 
