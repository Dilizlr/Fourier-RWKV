...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='its_model') 
        ...
        for epoch_idx in range(start_epoch, args.epochs + 1):
            ...
            for iter_idx, batch_data in enumerate(train_loader):
                ...
                if world_size > 1:
                    ...
                    metrics /= world_size
                    loss_avg, l_cont_avg, l_fft_avg = metrics[0].item(), metrics[1].item(), metrics[2].item()
                else:
                    loss_avg, l_cont_avg, l_fft_avg = loss.item(), l_cont.item(), l_fft.item()
            ...
            if is_main_process(rank) and (epoch_idx % Train['VAL_AFTER_EVERY'] == 0):
                ...
                with torch.no_grad():
                    ...
                    for inp, tar in val_loader:
                        ...
                        psnr_adder(custom_utils.torchPSNR_R(out, tar))
                        ssim_adder(custom_utils.torchSSIM_R(out, tar, H, W))
                ...
                if psnr > best_psnr:
                    ...
                    if epoch_idx >= 600:
                        ...
                        }, os.path.join(model_dir, f'model_epoch_PSNR_{best_psnr}_{ssim}_{epoch_idx}.pth'))
                ...
                if psnr >= 41.40:
                    torch.save({
                        ...
                        'state_dict': model.module.state_dict() if isinstance(model, DDP) else model.state_dict(),
                        'optimizer': optimizer.state_dict()
                    }, os.path.join(model_dir, f'model_epoch_PSNR_{psnr}_{ssim}_{epoch_idx}.pth'))
                ...
                if ssim > best_ssim:
                    ...
                    if epoch_idx >= 600:
                        torch.save({
                            ...
                            'state_dict': model.module.state_dict() if isinstance(model, DDP) else model.state_dict(),
                            'optimizer': optimizer.state_dict()
                        }, os.path.join(model_dir, f'model_epoch_SSIM_{best_ssim}_{psnr}_{epoch_idx}.pth'))

                print(f"PSNR: {psnr:.4f} (Best {best_psnr:.4f}) SSIM: {ssim:.4f} (Best {best_ssim:.4f})")

                # 保存周期模型
                if epoch_idx >= 400 and (epoch_idx % Train['SAVE_AFTER_EVERY'] == 0):
                    torch.save({
                        'epoch': epoch_idx,
                        'state_dict': model.module.state_dict() if isinstance(model, DDP) else model.state_dict(),
                        'optimizer': optimizer.state_dict()
                    }, os.path.join(model_dir, f'model_epoch_{epoch_idx}.pth'))
