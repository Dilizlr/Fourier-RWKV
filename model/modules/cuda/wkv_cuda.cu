...
__global__ void kernel_forward(const int B, const int T, const int C,
    ...
    // [MOD] 计算是否落在 B*C 范围内，避免无效线程越界访问
    const bool bc_valid = (idx < B * C);                                 // [MOD]
    const int _b = bc_valid ? (idx / C) : 0;                              // [MOD]
    const int _c = bc_valid ? (idx % C) : 0;                              // [MOD]

    const int _T = (T + TOKEN_SPLIT - 1) / TOKEN_SPLIT;
    const int _t = _T * token_id;
    const bool t_valid = (_t < T);                                        // [MOD]
    const bool active = bc_valid && t_valid;                              // [MOD]

    const int _offset = _b * T * C + _c;
    const int _tokenLength = active ? min(T - _t, _T) : 0;                // [MOD]

    F u = 0, w = 0;                                                       // [MOD]
    if (bc_valid) { u = _u[_c]; w = _w[_c]; }                             // [MOD]
    ...
    So2[channel_id][token_id] = active ? o2 : MIN_VALUE;                  // [MOD]
    Sa[channel_id][token_id] = active ? a  : F(0);                        // [MOD]
    Sb[channel_id][token_id] = active ? b  : F(0);                        // [MOD]
    ...
    // 再次写入共享内存，供后续后缀累积使用
    Sa[channel_id][token_id] = active ? c  : F(0);                        // [MOD]
    Sb[channel_id][token_id] = active ? d  : F(0);                        // [MOD]
    So2[channel_id][token_id] = active ? o1 : MIN_VALUE;                  // [MOD]
...
__global__ void kernel_backward(const int B, const int T, const int C,
    ...
    Sa[channel_id][token_id]  = active ? a  : F(0);                       // [MOD]
    Sb[channel_id][token_id]  = active ? b  : F(0);                       // [MOD]
    Sdadw[channel_id][token_id] = active ? dadw : F(0);                   // [MOD]
    Sdbdw[channel_id][token_id] = active ? dbdw : F(0);                   // [MOD]
    ...
    Sa[channel_id][token_id]    = active ? c    : F(0);                   // [MOD]
    Sb[channel_id][token_id]    = active ? d    : F(0);                   // [MOD]
    Sdadw[channel_id][token_id] = active ? dcdw : F(0);                   // [MOD]
    Sdbdw[channel_id][token_id] = active ? dddw : F(0);                   // [MOD]
    ...
    __syncthreads();
    // 写入分块梯度（无效写 0）
    Sdadw[channel_id][token_id] = (active ? gw : F(0));                   // [MOD]
    Sdbdw[channel_id][token_id] = (active ? gu : F(0));                   // [MOD]
    __syncthreads();

    // 聚合到 _gw/_gu：只允许合法 (b,c) 的 token_id==0 线程做累加
    if (bc_valid && token_id == 0){                                       // [MOD]
    ...
    Sa[channel_id][token_id] = active ? gc : F(0);                        // [MOD]
    Sb[channel_id][token_id] = active ? gd : F(0);                        // [MOD]
    So2[channel_id][token_id] = active ? go1 : MIN_VALUE;                 // [MOD]
    __syncthreads();
    ...
    __syncthreads();
    Sa[channel_id][token_id] = active ? ga : F(0);                        // [MOD]
    Sb[channel_id][token_id] = active ? gb : F(0);                        // [MOD]
    So2[channel_id][token_id] = active ? go2 : MIN_VALUE;                 // [MOD]
    __syncthreads();
...
void cuda_forward(int B, int T, int C, float *w, float *u, float *k, float *v, float *y) {
    ...
    // assert(B * C % threadsPerBlock.x == 0);                                // [MOD] 取消硬约束
    dim3 numBlocks( (B * C + threadsPerBlock.x - 1) / threadsPerBlock.x );     // [MOD] ceil 网格
