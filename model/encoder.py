...
from .modules.fftrwkv import FFTRWKV
...
class Encoder(nn.Module):
    def __init__(self, dim):
        ...
        # stage 1 
        self.enhanceBlock1 = FFTRWKV(patch_size=3, in_channels=self.dim, embed_dims=self.dim, depth=2)
        self.dw1 = Downsample(self.dim)


        # stage 2
        self.enhanceBlock2 = FFTRWKV(patch_size=3, in_channels=self.dim*2, embed_dims=self.dim*2, depth=3)
        self.dw2 = Downsample(self.dim*2)

        # stage 3
        self.enhanceBlock3 = FFTRWKV(patch_size=3, in_channels=self.dim*4, embed_dims=self.dim*4, depth=3)
        self.dw3 = Downsample(self.dim*4)

        # latent stage
        self.enhanceBlock4 = FFTRWKV(patch_size=3, in_channels=self.dim*8, embed_dims=self.dim*8, depth=4)
